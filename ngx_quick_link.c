...
ngx_int_t ngx_quick_data_init(ngx_quick_link_t *q_link, ngx_quick_layer_node_t *q_node) {
    q_node->data = ngx_calloc(sizeof(ngx_quick_data_t), q_link->log);
    
    q_node->data->keys = ngx_calloc(QUICK_LINK_DATA_SIZE * sizeof(char *), q_link->log);
    q_node->data->values = ngx_calloc(QUICK_LINK_DATA_SIZE * sizeof(char *), q_link->log);
...
ngx_int_t ngx_quick_data_add(ngx_quick_link_t *q_link, ngx_quick_data_t *q_data, char *key, int key_len, char *value, int value_len) {
    ...
    if (q_data->index == q_data->size) {
        ...
        char **new_keys = ngx_calloc(q_data->size * 2 * sizeof(char *), q_link->log);
        char **new_values = ngx_calloc(q_data->size * 2 * sizeof(char *), q_link->log);
    ...
    q_data->keys[q_data->index] = ngx_calloc((key_len + 1) * sizeof(char), q_link->log);
...
ngx_int_t ngx_quick_link_init(ngx_quick_link_t **q_link, ngx_log_t *log) {
    *q_link = ngx_calloc(sizeof(ngx_quick_link_t), log);
    
    (*q_link)->level = QUICK_LINK_DEFAULT_LEVEL;
    (*q_link)->count = 0;
    (*q_link)->log = log;
    (*q_link)->root = ngx_calloc(sizeof(ngx_quick_layer_node_t), log);
    ...
    while (i < QUICK_LINK_DEFAULT_LEVEL) {
        ...
        if(ptr->key == '\0') {
            ...
            if (ptr->key != key[i] && ptr->next == NULL) {
                ngx_quick_layer_node_t *new_node = ngx_calloc(sizeof(ngx_quick_layer_node_t), q_link->log);
                new_node->key = key[i];
                ptr->next = new_node;
                ptr = new_node;
            }
        ...
        if (i < QUICK_LINK_DEFAULT_LEVEL - 1 && ptr->next_layer == NULL) {
            ngx_quick_layer_node_t *new_layer = ngx_calloc(sizeof(ngx_quick_layer_node_t), q_link->log);
